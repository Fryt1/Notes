...
参考文档： 3.1.2 (渲染机制基础), 3.2.2 (从 Proxy 到 MeshBatch)
...
参考文档： 3.2.3 (从 MeshBatch 到 MeshDrawCommand), 3.3.3 (静态绘制路径)
...
- [x] **注册 Pass：** 在 `MeshPassProcessor.h` (或你的模块) 中注册一个新的 `EMeshPass::Type`，命名为 `DistortionPass`。

- [x] **编写处理器：** 复制 `FDepthPassMeshProcessor` (因为它最简单) 的代码，改名为 `FDistortionPassMeshProcessor`。
...
- [x] **渲染插入：** 找到 `FSceneRenderer::Render`，模仿 `RenderCustomDepthPass`，在合适的位置调用你的 `DistortionPass`。

- [x] **✅ 验收标准：** 用 RenderDoc 截帧，发现多了一个 Pass。当你移动“力场”位置时，该 Pass 内包含的 DrawCall 数量在动态变化。

---

## 📅 Phase 3: Shader 绑定与几何突变 (Week 5-6) [[📅 Phase 3 Shader 绑定与几何突变 (Week 5-6)]]

目标： 这里的代码将直接决定显卡如何绘制像素，实现“现实扭曲”的视觉效果。

参考文档： 3.2.3 (Shader 绑定相关代码)
...
- [x] **图元类型：** 查找 `FMeshDrawCommand.PrimitiveType`，了解 `PT_TriangleList`, `PT_LineList`, `PT_PointList` 的区别。
...
1. **问：** 这个 Proxy 里的数据（比如顶点），是它自己独占一份内存，还是大家共用一份？
    ...
    - _（针对 StaticMeshProxy，答案通常是：LOD 资源是共用的，但材质设置是自己独占的。）_
